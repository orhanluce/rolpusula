...
Before any path, including --section, check the working directory. It must contain
.rolpusula-workspace.json with format "rolpusula-workspace" and version 1, created
by the RolPusula init command. There must be no .git file or directory here or in
any ancestor. If either condition fails, stop without reading or writing candidate
documents and direct the user to run node bin/rolpusula.mjs init ../my-applications
...
>
> **Privacy note:** the files above now contain your personal data as plaintext.
> RolPusula creates this private workspace outside the product repository, with no
> Git repository or remote and all files ignored. Do not copy it into a public
> repository. Your operating system backup/sync settings still apply.
>
> **Try it out:**

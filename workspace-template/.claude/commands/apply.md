...
- Extract: **company name**, **role title**, **department** (if mentioned), **location**, **application deadline** (if the posting states one), and **language** of the posting (any language, including Turkish). Ask when the preferred output language is unclear.
...
- **Match the language of the job posting**, including Turkish, unless the user explicitly requests another language.
- Follow the structure from `06-cover-letter-templates.md`
- Use the `cover.cls` template
- Tailor the opening paragraph to the specific role and company
- Address to a named person if available in the posting, otherwise "Dear Hiring Manager" (or equivalent in posting language)
- Keep to approximately one page
- Mention AI tools only when the candidate's confirmed profile supports that experience. Never add a product name or a skill merely because this workflow uses it.
...
```bash
cd cv && lualatex -no-shell-escape -halt-on-error -interaction=nonstopmode main_<company>_<role>.tex
cd ../cover_letters && xelatex -no-shell-escape -halt-on-error -interaction=nonstopmode cover_<company>_<role>.tex
```
...
- Escape candidate and posting text as literal LaTeX content; never turn it into TeX commands, file includes or shell arguments. Keep shell escape disabled. This is not an OS sandbox: compile only reviewed local templates and keep unrelated private files out of scope.

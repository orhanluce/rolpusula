...
An AI-powered job application framework packaged by RolPusula for Claude Code,
OpenAI Codex, Gemini CLI, or a local Ollama model through Codex CLI. Keep this
private workspace outside Git; let the selected runtime evaluate postings,
tailor your CV, write cover letters, and prepare you for interviews.

> Note: This is an independent open-source project and is not affiliated with,
> endorsed by, sponsored by, or maintained by Anthropic, OpenAI, Google, or Ollama.
>
...
A structured workflow that turns the selected AI runtime into a job application assistant. The core workflow (self-profiling, fit evaluation, and the drafter-reviewer application pipeline) is **language- and country-agnostic**. The job portal search skills are built for the Danish market (Jobindex, Jobnet, Akademikernes Jobbank, etc.), but the pattern is designed to be swapped for your local job boards.
...
- One selected runtime: Claude Code, OpenAI Codex, Gemini CLI, or Codex CLI with Ollama. Start at [`ROL PUSULA - START HERE.md`](ROL%20PUSULA%20-%20START%20HERE.md); [`AGENTS.md`](AGENTS.md) maps each runtime to the canonical workflow.
- Python 3.10+
...
```bash
claude
# Then inside Claude Code or Gemini CLI:
/setup
```
...
6. **Compile and inspect** both PDFs: lualatex for the CV, xelatex for the cover letter. The selected runtime reads the rendered pages and iterates on the LaTeX until the CV is exactly 2 pages with no orphaned entry titles, and the cover letter is exactly 1 page with the signature visible and fonts consistent.
...
- **Drafter-reviewer separation.** The drafter writes; a fresh subagent critiques when the runtime supports one. Otherwise a clearly separated second pass is used and reported as such. The drafter then revises.
...
- RolPusula supports Claude Code, OpenAI Codex, Gemini CLI, and local Ollama inference.
